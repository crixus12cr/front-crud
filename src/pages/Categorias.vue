    ...
    <modal :show.sync="modalEdit">
      <card type="secondary">
        ...
        <div>
          ...
          <base-input label="Categoria" v-model="datosEdit.tipo_id">
            <select class="form-control" id="exampleFormControlSelect1" v-model="datosEdit.tipo_id">
              <option v-for="(option, index) in options" :key="index" :value="option.id">
                <p>{{ option?.nombre }}</p>
              </option>
            </select>
      ...
      <div class="col-lg-12 col-md-10">
        <card class="card">
          <ul class="nav nav-pills card-header-pills">
            <li class="nav-item">
              <h1 class="nav-link">Categorias</h1>
            </li>
...
export default {
  ...
  data() {
    return {
      ...
      options: [],
      nCancion: null,
      datosEdit: { nombre: '', tipo_id: 0, estado: 0 }
    }
  },
  created() {
    this.consulta()
    axios.get(`${process.env.VUE_APP_BASE_URL}/tipo_cancion`, null, {
      headers: headers
    }).then(resp => {
  ...
  methods: {
    ...
    editEnd() {
      axios.put(`${process.env.VUE_APP_BASE_URL}/categorias/${this.datosEdit.id}`, { nombre: this.datosEdit.nombre, tipo_id: this.datosEdit.tipo_id, estado: this.datosEdit.estado }, {
        headers: headers
      }).then(resp => {
    ...
    add() {
      axios.post(`${process.env.VUE_APP_BASE_URL}/categorias`, { nombre: this.nCancion, tipo_id: this.categoria }, {
        headers: headers
      }).then(resp => {
        alert(resp.data.message);
        this.modal = false
    ...
    borrar(item) {
      axios.delete(`${process.env.VUE_APP_BASE_URL}/categorias/${item.id}`, null, {
        headers: headers
      }).then(resp => {
        alert(resp.data.message);
        this.consulta()
